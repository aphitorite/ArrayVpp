...
public abstract class LassoSorting extends Sort {
    ...
    protected BinaryInsertionSort binserter;
    ...
    protected int linearExpSearch(int[] array, int start, int end, int key, boolean goRight, double sleep) {
    	Range exp = exponentialSearch(array, start, end, key, sleep, goRight);
    	if(goRight) {
			while(end >= start) {
    ...
    protected void rotateMerge(int[] array, int start, int mid, int end) {
		if(start==mid||mid==end)
			return;
		if(end-start < 4) {
			this.binserter.customBinaryInsert(array, start, end, 1);
			return;
		}
    ...
    protected int grabKeys(int[] array, int len, int idealKeys) {
    	int keys = 0, uniqueKeys = 0, end = EctaKeyLoc+BufSize, searchPosition = end-1;
    ...
    protected void mergePingPongFW(int[] array, int start, int mid, int end, int buffer) {
    ...
    protected void mergePingPongBW(int[] array, int start, int mid, int end, int buffer) {
    	int left = mid-1, right = end-1, len = end-start;
    	while(left >= start && right >= mid) {
    		buffer--;
    		if(Reads.compareValues(array[left], array[right]) > 0) {
    			Writes.swap(array, left--, len + buffer, 1, true, false);
    		} else {
    			Writes.swap(array, right--, len + buffer, 1, true, false);
    		}
    	}
    	while(left >= start)
			Writes.swap(array, left--, len + --buffer, 1, true, false);
    ...
    public void common(int[] array, int start, int end) {
    	...
    	if(len <= 4) {
    		...
    		this.binserter.customBinaryInsert(array, start, end, 0.5);
    		return;
    	}
    	Statistics.putStat("Store");
    	BufSize = this.ceilSqrt(len);
    	BufLoc = end-2*BufSize;
    	EctaKeyLoc = end-BufSize;
    	this.KeySpace = Writes.createExternalArray(BufSize);
    	int k = this.grabKeys(array, len, 2*BufSize);
    	...
    	for(int j=4; j<=BufSize/2; j*=4) {
    		for(int i=start; i<BufLoc; i+=4*j) {
    			if(i+j>=BufLoc)
    				break;
    			if(2*j>=BufSize) {
					this.mergeRight(array, i, i+j, i+2*j);
					i-=2*j;
					continue;
    			}
    			if(i+4*j<=BufLoc) {
    				this.mergePingPongFW(array, i, i+j, i+2*j, BufLoc);
    				this.mergePingPongFW(array, i+2*j, i+3*j, i+4*j, BufLoc+2*j);
    				this.mergePingPongBW(array, BufLoc, BufLoc+2*j, BufLoc+4*j, i);
    			} else if(i+2*j<=BufLoc) {
    				this.mergePingPongFW(array, i, i+j, i+2*j, BufLoc);
    				this.multiSwapBW(array, i, BufLoc, 2*j);
    				if(i+3*j<=BufLoc)
    					this.mergeRight(array, i, i+2*j, i+3*j);
    			} else if(i+j<BufLoc) {
    				this.mergePingPongFW(array, i, i+j, BufLoc, BufLoc);
    				this.multiSwapBW(array, i, BufLoc, BufLoc-i);
    			}
    		}
    	}
    	while(BufLoc>start) {
    	...
    	this.binserter.customBinaryInsert(array, BufLoc, BufLoc+BufSize, 0.5);
