...
public class NilSort extends GrailSorting {
	...
	public void zeromerge(int[] array, int start, int mid, int end) {
		...
		if(Reads.compareIndices(array, start, end-1, 5, true) > 0) {
			IndexedRotations.neon22(array, start, mid, end, 1, true, false);
			return;
		}
		IndexedRotations.neon22(array, buf, buf+bufsz, start, 1, true, false);
		...
		while(l < mid || r < end) {
			...
			IndexedRotations.neon22(array, lBuf, mid, rBuf+rSz, 2.5, true, false);
			l += rBuf-mid+rSz;
			lBuf += rBuf-mid+rSz;
			mid = r = rBuf+rSz;
			rSz=0;
