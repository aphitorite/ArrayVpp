...
final public class CbrtGrailSort extends GrailSorting {
    ...
    private void pingPongMergeBW(int[] array, int start, int mid, int end) {
    	int l=mid-1, h=end-1;
    	while(l>=start&&h>=mid) {
    		if(Reads.compareValues(array[l], array[h]) > 0) {
    			Writes.swap(array, bufferLen+--bufferLoc, l--, 1, true, false);
    		} else {
    			Writes.swap(array, bufferLen+--bufferLoc, h--, 1, true, false);
    		}
    	}
    	while(l>=start)
			Writes.swap(array, bufferLen+--bufferLoc, l--, 1, true, false);
    ...
    private int[] fragMergeFW(int[] array, int start, int mid, int end, int tail) {
    	int l=start, h=mid;
    	while(l<mid&&h<end) {
    		int c = Reads.compareValues(array[l], array[h]);
    		if(c < 0 || (tail == 1 && c == 0)) {
    			Writes.swap(array, bufferLoc++, l++, 1, true, false);
    		} else {
    			Writes.swap(array, bufferLoc++, h++, 1, true, false);
    	...
    	if(l<mid) {
    		...
    		return new int[] {end-f, tail};
    	} else {
    		return new int[] {h, tail ^ 1};
    	}
    }
    ...
    private void blockSelectFW(int[] array, int start, int mid, int end) {
    	...
    	int blockLen1 = Math.max((2*blockLen0)/keysHighLen, bufferLen),
    		leftoverLeft = 0, leftoverRight = 0, leftoverTotal = 0, tail = 1;
    	...
    	if(blockLen0 > bufferLen) {
    		for(int k=start, tK = keysLowLoc; k<=end-blockLen0; k+=blockLen0, tK++) {
    			boolean swapped=true;
    			for(int l=k, tL = tK; swapped&&l>=start; l-=blockLen0, tL--) {
    				swapped=false;
	    			for(int i=l, iKeyHi = keysHighLoc; i<=end-blockLen1 && i<l+2*blockLen0; i+=blockLen1, iKeyHi++) {
	    				int min=i, minKeyHi=iKeyHi;
	    	    		for(int j=i+blockLen1, jKeyHi = iKeyHi+1; j<=end-blockLen1 && j<l+2*blockLen0; j+=blockLen1, jKeyHi++) {
	    	    			int cmp=Reads.compareIndices(array, min, j, 0.125, true);
	    	    			if(cmp > 0 || (cmp == 0 && Reads.compareIndices(array, minKeyHi, jKeyHi, 0.5, true) > 0)) {
	    	    				min=j;
	    	    				minKeyHi=jKeyHi;
	    	    			}
	    	    		}
	    	    		if(i!=min) swapped=true;
	    	    		if(((minKeyHi - keysHighLoc) < keysHighLen / 2 || (iKeyHi - keysHighLoc) >= keysHighLen / 2)
	    	    			|| Reads.compareIndices(array, tL, tL+1, 2.5, true) > 0) {
	    	    			multiSwap(array, i, min, blockLen1);
	    	    			Writes.swap(array, minKeyHi, iKeyHi, 1, true, false);
	    	    		}
	    			}
    	...
    	for(int i=1; i<keys0; i++) {
        	if(blockLen0 > bufferLen) {
        		int v=start+(i-1)*blockLen0;
        		int mal = Math.min(blockLen0, end-(v+blockLen0));
        		if(mal < 0)
        			break;
        		grailMergeWithoutBuffer(array, v, blockLen0, mal);
        	} else {
        		int[] info = fragMergeFW(array, frag, start+i*blockLen0, Math.min(start+(i+1)*blockLen0, end), tail);
        		frag = info[0];
        		tail = info[1];
        	}
    	}
    	...
    	grailMergeWithoutBuffer(array, end, leftoverLeft, leftoverRight);
    	IndexedRotations.holyGriesMills(array, bufferLoc, bufferLoc+bufferLen, end+leftoverTotal, 1, true, false);
    ...
    public void common(int[] array, int start, int end) {
    	...
    	keysLowLen = keys0;
    	bufferLen = keys2;
    	//if(keysHighLen < keysNeeded || keysLowLoc < keysNeeded) {
    	...
    	for(int i=4; i<=bufferLen; i*=2) {
    		if(right)
    			...
    			for(int j=bufferLoc; j>bufferStart; j-=2*i) {
    				int k=Math.max(j-2*i, bufferStart);
    				lastBlock=bufferLoc+bufferLen;
    				pingPongMergeBW(array,k,Math.max(j-i, bufferStart),j);
    			}
    		}
    	...
    	if(!right) {
    		IndexedRotations.holyGriesMills(array, bufferStart, bufferLoc, bufferLoc+bufferLen, 1, true, false);
    		bufferLoc=bufferStart;
    		lastBlock=bufferLoc+3*bufferLen;
    	}
		int offset=lastBlock;
