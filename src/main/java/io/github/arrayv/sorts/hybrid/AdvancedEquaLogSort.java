...
final public class AdvancedEquaLogSort extends Sort {
	public AdvancedEquaLogSort(ArrayVisualizer arrayVisualizer) {
		super(arrayVisualizer);
		
		this.setSortListName("Advanced Equalog");
	...
	private static final boolean CLEAN_BUFFER = true;
	...
	// uses a tweaked implementation of aphiBP typing, which makes fewer writes than Aeos at the cost of a guaranteed redistribution
	// phase.
	// worstcase writes (aeos):      1.5n type + 0.5n tag + 1n sort + 0.5n index + 0.5n untag + 0.5n redist
	//                             = 4.5n writes
	// worstcase writes (currently): 1n type + 0.5n tag + 1n sort + 0.75n index/untag + 0.5n redist + roughly O(n) fragment handling
	//                             = ~3.75n writes
	//
	private int[] partition(int[] array, int[] tmp, int a, int B, int p, int c, final int s) {
		int b = B - (B - a) % s, domain = 0;
		boolean skipop = true;

		if(a == b) { // easy partition
			int tt = 0;
			for(int i = a; i < B; i++) {
				int cv = Reads.compareIndexValue(array, i, p, 0.5, true);
				skipop = skipop && (!(c == 0 ^ cv < c) || cv == 0);
				domain |= cv;
				if(cv < c) {
					if(tt > 0) {
			...
			return new int[] {B - tt, (domain==0?1:0)|(skipop?2:0)};
		}
		...
		int id = a, t = 0,  // aeos/muku vals 
			...
			ii = -1;        // tracker index
		while(id < b && t < s) {
			int cv = Reads.compareIndexValue(array, id, p, 0.5, true);
			skipop = skipop && (!(c == 0 ^ cv < c) || cv == 0);
			domain |= cv;
			if(cv < c) {
				if(lc < 0) lc++;
		...
		if(b < B) {
			...
			while(id < B) {
				...
				skipop = skipop && (!(c == 0 ^ cv < c) || cv == 0);
				domain |= cv;
				if(cv < c) { // store in buffer space left by block fragments
					if(lo++ == fm) { // overflow
						v = a + be_r;
					}
					Writes.write(array, v, array[id], 2, true, false);
					if(CLEAN_BUFFER && id != v) Writes.visualClear(array, id);
					v++;
				} else {
					if(thold < s) { // store in unfilled aux if available [safeguard]
						...
						if(CLEAN_BUFFER && id != vr) Writes.visualClear(array, id);
						vr++;
					}
				}
				id++;
		...
		return new int[] {mid, (domain==0?1:0)|(skipop?2:0)};
	}
	...
	public void advLogSort(int[] array, int[] tmp, int a, int b, int s, int depth) {
		...
		while(b - a > 32) {
			...
			mi = partition(array, tmp, a, b, p, 1, s);
			m = mi[0];
			if((mi[1] & 1) > 0) return; // all equals
			if((mi[1] & 2) > 0) {
				a = m;
				continue;
			}
			if(m == b) { // pivot is highest rank
				...
				b = partition(array, tmp, a, b, p, 0, s)[0];
				continue;
			}
