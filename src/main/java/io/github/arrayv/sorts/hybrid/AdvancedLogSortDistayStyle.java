...
final public class AdvancedLogSortDistayStyle extends Sort {
	public AdvancedLogSortDistayStyle(ArrayVisualizer arrayVisualizer) {
		super(arrayVisualizer);
		
		this.setSortListName("Advanced Log (Colorcoded)");
		...
		if(b < B) {
			...
			while(id < B) {
				if(Reads.compareIndexValue(array, id, p, 0.5, true) < c) { // store in buffer space left by block fragments
					...
					if(id != v) {
						if(CLEAN_BUFFER) Writes.visualClear(array, id);
	    				else Highlights.colorCode("buffer", id);
					}
					Highlights.colorCode("fraglo", v);
					v++;
				} else {
					if(thold < s) { // store in unfilled aux if available [safeguard]
						...
						Highlights.colorCode("fraghi", vr);
						vr++;
					}
				}
				id++;
			...
			if(rb == 0) { // missing full buffer
				...
				if(lo <= fm) {
    				...
    				arraycopy_CSSC(tmp, 0, array, v, t, 1, true, false, "bufhi");
    				rotate(array, a + lb * s, a + be_r, v + t);
				}
			}
			mid = a + bm + lo;
		} else {
