...
final public class UnstableLogPartitionSort extends GrailSorting {
    ...
    private int ninther(int[] array, int a, int b) {
    	...
    	int len = b - a, half = len / 2, quart = len / 4, eight = len / 8, fallback = len / 16;
    ...
    private int idx(int[] array, int v, int p, int a1, int z, int bufcnt, boolean iv) {
    	return v<a1+(bufcnt*(z+1)*z) && (Reads.compareIndices(array, v, p+v - a1, 1, true) > 0 ^ iv) ? p + v - a1 : v;
    }
    ...
    private void why(int[] array, int a, int b, int p, int m, boolean iv) {
    	...
    	if(z+2*(z+1)*z>m) {
    		// we need to fix
    		System.out.println("Better Call ~~Saul~~Distray");
    		return;
    	}
    	...
    	// count elements
    	for(int i=a1; i<b; i++) {
    		int j = b(array, a, a1, idx(array, i, p, a1, z, 1, iv), false) - a;
    		c.incr(j);
    	}
    ...
    private void run(int[] array, int a, int b) {
    	...
    	if(l > r) {
    		...
    		merge(array, m2, b, a);
    		why(array, a, m1, m1, b-m1, false);
    	} else {
    		// merge left side, bucketsort right side
    		merge(array, a, m1, m2);
    		why(array, m2, b, a, m2-a, true);
    	}
    }
