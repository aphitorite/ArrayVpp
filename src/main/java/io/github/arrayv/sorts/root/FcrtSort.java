...
public final class FcrtSort extends Sort {
   private InsertionSort insertSorter;

   public FcrtSort(ArrayVisualizer arrayVisualizer) {
   ...
   public void sqrtCommonSort(int[] arr, int pos, int len, int[] extBuf, int extBufPos, int[] tags, boolean auxwrite) {
      ...
      if (len <= 16) {
         ...
         while (blockLen * Math.sqrt(blockLen) < len) {
            blockLen *= 2;
         }
   ...
   public void runSort(int[] array, int len, int bucketCount) {
      ...
      while (bufferLen * Math.sqrt(bufferLen) < len) {
         bufferLen *= 2;
      }
