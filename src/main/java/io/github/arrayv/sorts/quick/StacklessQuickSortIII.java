...
final public class StacklessQuickSortIII extends Sort {
    public StacklessQuickSortIII(ArrayVisualizer arrayVisualizer) {
        super(arrayVisualizer);
        
        this.setSortListName("Stackless Quick III");
    ...
    private void quickSort(int[] array, int l, int r) {
    	int a = l, b = r, s = r - 1, d, c, m[] = new int[2], mq;
    	BinaryInsertionSort ins = new BinaryInsertionSort(arrayVisualizer);
    	while(a < b - 16) {
    		d = mq = 0;
    		do {
    			c = b;
    	    	while(a < b - 16) {
    	    		...
    	    		if(d++ == 0) {
    	    			...
    	    			Writes.swap(array, m[0], s, 1, true, false);
    	    		}
    	    		b = c = m[0];
    	    	}
    	    	d--;
    	    	if(mq == 0)
    	    		ins.customBinaryInsert(array, a, c, 0.25);
    	    	a = b;
    	    	if(d > 1) {
	    	    	b = bs(array, a+1, s, array[b]);
    	    	} else if(d == 1) {
    	    		b = s;
    	    	} else {
    	    		b = r;
	        	...
	        	if(++a<r&&d>0) Writes.swap(array, a-1, s, 1, true, false);
    		} while(d > 0);
    	}
    	ins.customBinaryInsert(array, a, r, 0.25);
    }
